...
import tensionbox_window            ## edit in Qt Designer, convert with pyuic5
...
def plotadc(y0):
    ...
    fftnumber0 = 1./(len(y0)*SampleRate)
...
class TensionBox(QMainWindow, tensionbox_window.Ui_MainWindow):
    """
    ...
    def testplot3(self):
        ...
        self.canvas = DataCanvas(self.data_widget,data=self.z,width=5, height=4, dpi=100,
                                 xlabel='Wire Number',ylabel='Tension [g]')
    ...
    def openFile(self):
        ...
        filelabel = QFileDialog.getOpenFileName(self, 'Open File', "", 'Text Files (*.txt)')
        self.fileName.setText("Saving data to: "+filelabel[0].split("/")[-1])
    ...
    def run(self,nextstraw=False):
        ...
        thus obtains the frequency of the straw's vibrations. Computes the tension. After each run, writes a line with
        the time, straw number, straw length, temperature, relative humidity, frequency, pulse width used, and tension
        to the output file.
        """
        ...
        for i in range(0, self.SpinNiter.value()):
            ...
            if straw_flag == 0:
                ...
                tension = 4*(length**2)*mu*(freq**2)*(100/grav_accel) # tension in grams
            elif straw_flag == 1:
                # For a straw
                tension = 1000*((freq-C/length**2)**2)*(2*length/K)**2 #tension in grams
            else:
                tension = 0
...
if __name__ == '__main__':
    ## fix for distinguishing Arduino types when multiple devices are connected
    ## (also works on General Nanosystems where Arduinos recognized as "USB Serial")
    ## assumes tensionbox is running with Arduino Due -> cut on hardware ID
